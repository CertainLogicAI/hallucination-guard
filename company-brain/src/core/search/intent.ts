...
import { classifyCertainLogicIntent, certainLogicIntentToDetail } from './certainlogic-intent.ts';
 ...
 *
 * CertainLogic intents checked after base patterns so temporal/event/entity
 * queries win when they overlap, but CL-specific patterns still resolve.
 */
export function autoDetectDetail(query: string): 'low' | 'medium' | 'high' | undefined {
  // Base patterns first
  const baseDetail = intentToDetail(classifyQueryIntent(query));
  if (baseDetail !== undefined) return baseDetail;

  // CertainLogic patterns for general queries
  const clIntent = classifyCertainLogicIntent(query);
  if (clIntent) {
    return certainLogicIntentToDetail(clIntent);
  }
